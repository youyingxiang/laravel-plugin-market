            ...
            <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg  px-5 px-2 bg-white">
                ...
                <div>
                  <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                      ...
                      <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <input type="text" v-model="version.version" id="plugin-version" autocomplete="given-name" class="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
                      </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                      ...
                      <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <select id="country" name="country" @change="changeStatus" autocomplete="country-name" class="max-w-lg block focus:ring-indigo-500 focus:border-indigo-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
                          <template v-for="item in status" >
                            <option v-if="item.value === version.status"  :value="item.value" selected>{{item.text}}</option>
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                      ...
                      <div class="relative rounded-md shadow-sm sm:mt-0 sm:col-span-2">
                        ...
                        <input type="number" id="price" min=0 v-model="version.price" class="focus:ring-indigo-500 focus:border-indigo-500  max-w-lg block w-full pl-7 pr-12 sm:max-w-xs sm:text-sm border-gray-300 rounded-md" placeholder="0.00">
                      </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                      ...
                      <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <textarea id="about"  rows="3" v-model="version.description" class="max-w-lg shadow-sm block w-full focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border border-gray-300 rounded-md"></textarea>
                      </div>
                    </div>
                          ...
                          <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                              <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                                <input id="file-upload" accept="image/*" type="file" @change="upload" ref="upload" class="sr-only">
                              </label>
                            </div>
              ...
              <div class="py-5">
                ...
                <div class="flex justify-end">
                  ...
                  <button type="button" :disabled="isLoading" @click="save()" class="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
  ...
  methods: {
    ...
    changeStatus(event) {
      if (event.target.value) {
        this.version.status = event.target.value
      }
    },
    upload(event) {
